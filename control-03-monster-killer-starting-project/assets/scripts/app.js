...
function writeToLog(ev, val, monsterHealth, playerHealth) {
    let logEntry = { // every log entry has these fields.
        event: ev,
        value: val,
    ...
    } else if (event === LOG_EVENT_PLAYER_STRONG_ATTACK) { // for readability
        logEntry = {
            event: ev,
            value: val,
            target: 'MONSTER',
    ...
    } else if (event === LOG_EVENT_GAME_OVER) { // if use shorter code option, no need for this block since there is no TARGET value to pass.
        logEntry = {
            event: ev,
            value: val,
            finalMonsterHealth: monsterHealth,
...
function endRound() {
	...
	if (currentMonsterHealth <= 0 && currentPlayerHealth > 0) {
        ...
        writeToLog(
            LOG_EVENT_GAME_OVER, // event - state of game
            'PLAYER WON', // game end message
